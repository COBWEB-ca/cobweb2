...
/**
 * BehaviorArray is a class designed to take a collection of bits as input and
 ...
 * from previous timestep) 4) m bits for communication code received (m equal to
 * communication from output) 5) 2 bits for orientation
 * (N(00),S(01),E(10),W(11)) 6) 2 bits for agent's current energy level ==
 * floor(E/40) and no more than 3 Output: A collection of 4 outputs: 1) 2 bits
...
public class BehaviorArray {
	...
	/**
	 * Constructor: Creates a behavior array that can accept an amount of input
	 * bits specified by the first parameter, and stores groups of output bits
	 * who's number and individual sizes are specified by the second parameter.
	 */
	public BehaviorArray(int input, int[] output) {
		...
		/*
		 * totalBits is equal to the output size times the number of
		 * permutations for the input bits
		 */
		totalBits = outputBits * size;

		/*
		 * totalInts is equal to the totalbits divided by 32 (there's 32 bits in
		 * an int) plus 1 since we might have a partial int used and the shift
		 * operation will round down. Certain methods also assume the existance
		 * of this extra int
		 */
	...
	/**
	 * @return an array filled with the different outputs associated with the
	 *         element number given as a parameter
	 */
	...
	/**
	 * Splice takes another behavior array as a parameter and genetically
	...
	public BehaviorArray splice(BehaviorArray BOther) {
		...
		/*
		 * Fills boolArray up in a manner such that a random half of the
		 * elements are 'true'
		 */
		for (int i = 0;;) {
		...
		/*
		 * Set the values of our new array according the values in boolArray. An
		 * element is from 'this' array if the corresponding element in
		 * boolArray is false and from the 'other' array (supplied as an
		 * arguement) if its true
		 */
	...
	public int get(int index) {
		...
		/*
		 * base is the first int value of the array that contains bit(s) of the
		 * target element
		 */
		int base = bitbase >> 5;
		/*
		 * basemod is the number of the first bit of the target element in the
		 * int value
		 */
		...
		long buff = (array[base] & 0xFFFFFFFFL)
				| ((long) array[base + 1] << 32);
	...
	public long compareTo(Object other) {
		...
		/*
		 * compare each int element is much faster than comparing each output
	...
	public BehaviorArray copy(float mutationRate) {
		...
		for (int i = 0; i < size; ++i) {
			...
			for (int j = outputSize.length - 1; j >= 0; --j) {
				/***************************************************************
				 * *****This used to make the mutation based on behaviour array
				 * if( cobweb.globals.random.nextFloat() <= mutationRate ) {
				 * outputCode.add( cobweb.globals.random.nextBits( outputSize[j] ),
				 * outputSize[j] ); } else { outputCode.add( get[j],
				 * outputSize[j] ); }
				 */
	 ...
	 * return new java.awt.Color( colorMap.getSpace(), A, 1.0f
	 * ).brighter().brighter(); }
	 */

	/*
	 * public java.awt.Color getColor() { float A[] = generateColor( 3, 0,
	 * totalBits ); return new java.awt.Color( A[0]*2.0f, A[1]*2.0f, A[2]*2.0f );
	 * //return new java.awt.Color( A[0], A[1], A[2]).brighter().brighter();
	 *  }
	 */

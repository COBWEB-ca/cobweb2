...
public class ComplexAgent extends cobweb.Agent implements cobweb.TickScheduler.Client { 
   ...
   public static void setDefaultMutableParams(float[] d_mutationRate, int[] d_energy, int[] d_foodEnergy, int[] d_otherFoodEnergy,
		   int[] d_breedEnergy, int[] d_pregnancyPeriod, int[] d_stepEnergy, int[] d_stepRockEnergy, int[] d_turnRightEnergy,
		   int[] d_turnLeftEnergy, int[] d_commSimMin, int[] d_stepAgentEnergy, int[] d_sexualPregnancyPeriod, float[] d_breedSimMin,
		   float[] d_sexualBreedChance, float[] d_asexualBreedChance, int[] d_agingLimit, float[] d_agingRate, int[] d_pdCoopProb,
		   int[] d_broadcastFixedRange, int[] d_broadcastEnergyMin, int[] d_broadcastEnergyCost) {
...
// private long id;
  ...
  public int type() { return agentType; }
  /* energy gauge */
  private int energy;
  ...
  public long birthday() { return birthTick; }
  private long age = 0;
  ...
  private double energyPenalty(boolean log) {
    if (!agingMode) return 0.0; 
    Double age = new Double (currTick - birthTick);
    if (tracked && log) info.useExtraEnergy(agentType, Math.min(Math.max(0, energy), (int)(agingRate*(Math.tan(((age.doubleValue()/agingLimit)*89.99)*Math.PI/180)) + 0.5))); 
    return Math.min(Math.max(0, energy), agingRate*(Math.tan(((age.doubleValue()/agingLimit)*89.99)*Math.PI/180)));
  }
  public static void clearData() {
    if (tracked) ComplexAgentInfo.resetGroupData();
  }
  public static void dumpData(long tick) {
    if (tracked) ComplexAgentInfo.dumpGroupData(tick, writer);
  }
  ...
  public void die () {
	GATracker.removeAgent(this); // Remove this agent from the GATracker's list of agent (presumably all living agents)
  ...
  public void getDrawInfo(cobweb.UIInterface theUI) { 
    ...
    // based on agent type
    if (colorful){ 
      theUI.newAgent(getColor(),
                     colorMap.getColor( agentType, 1 ), stratColor,     
                     new java.awt.Point(getPosition().v[0], getPosition().v[1]), 
                     new java.awt.Point(facing.v[0], facing.v[1]));
    }else{
      
      theUI.newAgent(java.awt.Color.red, stratColor, 
                     new java.awt.Point(getPosition().v[0], getPosition().v[1]), 
                     new java.awt.Point(facing.v[0], facing.v[1]));
    } 
  }
...
private static class SimpleController implements cobweb.Agent.Controller { 
  ...
  public double similarity( cobweb.Agent other ) {
    if( ! (other instanceof ComplexAgent) ) 
      return 0.0;
    return ((GeneticController)controller).similarity( (GeneticController)((ComplexAgent)other).getController() ); 
  }
  
  public double similarity( int other ) {
    return ((GeneticController)controller).similarity( other); 
  }
  ...
  private static class GeneticController implements cobweb.Agent.Controller { 
    ...
    // sexual reproduction
    public GeneticController(GeneticController parent, GeneticController parent2, float mutationRate) {
      ga = (BehaviorArray)(parent.ga.splice(parent2.ga).copy(mutationRate));
      memorySize = parent.memorySize;
    }
    ...
    public GeneticController (int memory, int comm) {
      ...
      ga = new BehaviorArray( INPUT_BITS+memorySize+commSize, outputArray );
      ga.init(); 
    }   
    ...
    public void controlAgent (cobweb.Agent baseAgent) {
      ...
      if( theAgent.energy > ENERGY_THRESHOLD ) 
        ...
        inputCode.add( (int)((double)theAgent.energy/(ENERGY_THRESHOLD) * 4.0), 2 );
    ...
    public GeneticController splice( GeneticController other ) {
      return new GeneticController( (BehaviorArray)ga.splice(((GeneticController)other).ga), memorySize );
    }
...
private static long groupLastPolled = 1;
  ...
  public void tickNotification (long tick) {
    ...
    if (birthTick == 0) birthTick = currTick; 
    age++;
    ...
    if (broadcastMode & !ComplexEnvironment.currentPackets.isEmpty()) receiveBroadcast();// []SK
  }
  ...
  public long look () {
    ...
    if (destPos == null) return ComplexEnvironment.FLAG_STONE;
    // Check for stone...
    if (destPos.testFlag(ComplexEnvironment.FLAG_STONE)) return ComplexEnvironment.FLAG_STONE;
    // If there's another agent there, then return that it's a stone...
    if (destPos.getAgent() != null) return ComplexEnvironment.FLAG_STONE;
    // If there's food there, return the food...
    if (destPos.testFlag(ComplexEnvironment.FLAG_FOOD)) return ComplexEnvironment.FLAG_FOOD;
    // waste check
    if (destPos.testFlag(ComplexEnvironment.FLAG_WASTE)) return ComplexEnvironment.FLAG_WASTE;
  ...
  boolean canStep () {
    ...
    if (destPos.testFlag(ComplexEnvironment.FLAG_STONE)) return false;
    // and clear of wastes
    if (destPos.testFlag(ComplexEnvironment.FLAG_WASTE)) return false;
    // as well as other agents...
    if (destPos.getAgent() != null) return false;
    return true;
  }
...
// System.out.println("Agent " + getID() + " found food at location: " + destPos
// + ", energy: " + energy);
        ...
        } else if ( !pregnant && asexFlag && energy >= breedEnergy && asexualBreedChance != 0.0 && cobweb.globals.random.nextFloat() < asexualBreedChance) { 
      ...
      if (breedPos != null) {
        ...
        if( breedPartner == null ) {
          ...
          // child's strategy is determined by its parents, it has a 50%
...
// boolean choose = generator.nextBoolean();
      ...
      if(this.agentPDAction != -1){// $$$$$ if playing Prisoner's Dilemma. Please refer to ComplexEnvironment.load, "// spawn new random agents for each type"
        want2meet = true;
      }
      ...
      if( adjacentAgent.agentType == agentType ) {
        ...
        double sim = 0.0; 
        boolean canBreed = !pregnant && energy >= breedEnergy && sexualBreedChance != 0.0 && cobweb.globals.random.nextFloat() < sexualBreedChance; 
        
        // Generate genetic similarity number 
        try {
        	sim = GeneticCode.compareGeneticSimilarity(this.getGeneticSequence(), adjacentAgent.getGeneticSequence());
        ...
        if( canBreed && sim >= breedSimMin && ((want2meet && adjacentAgent.want2meet) || (agentPDAction == -1))) {
          pregnant = true;
          pregPeriod = sexualPregnancyPeriod;
          breedPartner = adjacentAgent;
        } 
      }      
      // perform the transaction only if non-pregnant and both agents want to
		// meet
      if(!pregnant && want2meet && adjacentAgent.want2meet) {
        
        //cobweb.UIInterface ui = ComplexEnvironment.getUIPipe();  // $$$$$$ this silenced line was for writing PD info into the output window.  Apr 22
        ...
        lastPDMove = adjacentAgent.agentPDAction; // Adjacent Agent's action
        ...
        /*****  $$$$$$ This silenced block was used for updating PD info in the output window.  Apr 22
        String Strategy = null;     // $$$$$ this String used to be used for the Output Window's updating.  Apr 18
        if (pdTitForTat) { // remove this if-block later
          //agentPDStrategy = 1;
          Strategy = "TitForTat";
        }
        else {
        ...
        String adjStrategy = null;    // $$$$$ this String used to be used for the Output Window's updating.  Apr 18
        if (adjacentAgent.pdTitForTat) { // remove this if-block later
          //adjacentAgent.agentPDStrategy = 1;
          adjStrategy = "TitForTat";
        }
        else {
          //adjacentAgent.agentPDStrategy = 0;
          adjStrategy = "Probability";
        }
        */
        
        /*
		 * TODO The ability for the PD game to contend for the Get the food
		 * tiles immediately around each agents: . . . . . . . . . . . . . . . . . . .
		 * X X X X . . . > < . . -> . X > < X . . . . . . . . X X X X . . . . . . . . . . . . .
		 * 
       ...
       /*
        ...
        /* REWARD */
        if (agentPDAction == PD_COOPERATE && adjacentAgent.agentPDAction == PD_COOPERATE) { 
   ...
   /*
	 ...
	 * ("+position.v[0]+","+position.v[1]+") COOPERATED and gained
	 * "+ComplexEnvironment.PD_PAYOFF_REWARD+" energy unit(s)\n");
	 * ui.writeToTextWindow(" Type: "+(type()+1)+" Strategy: "+Strategy+"\n");
	 * ui.writeToTextWindow(" Agent "+(adjacentAgent.getID())+"
	 * ("+adjacentAgent.getPosition().v[0]+","+adjacentAgent.getPosition().v[1]+")
	 * COOPERATED and gained "+ComplexEnvironment.PD_PAYOFF_REWARD+" energy
	 * unit(s)\n"); ui.writeToTextWindow(" Type: "+(adjacentAgent.type()+1)+"
	 * Strategy: "+adjStrategy+"\n");
	 */
          
          /* SUCKER */
        } else if (agentPDAction == PD_COOPERATE && adjacentAgent.agentPDAction == PD_DEFECT) {
          energy += ComplexEnvironment.PD_PAYOFF_SUCKER; adjacentAgent.energy += ComplexEnvironment.PD_PAYOFF_TEMPTATION;
          
          /*
			 * ui.writeToTextWindow(" Agent "+getID()+"
			 * ("+position.v[0]+","+position.v[1]+") COOPERATED and gained
			 * "+ComplexEnvironment.PD_PAYOFF_SUCKER+" energy unit(s)\n");
			 * ui.writeToTextWindow(" Type: "+(type()+1)+" Strategy:
			 * "+Strategy+"\n"); ui.writeToTextWindow(" Agent
			 * "+(adjacentAgent.getID())+"
			 * ("+adjacentAgent.getPosition().v[0]+","+adjacentAgent.getPosition().v[1]+")
			 * DEFECTED and gained "+ComplexEnvironment.PD_PAYOFF_TEMPTATION+"
			 * energy unit(s)\n"); ui.writeToTextWindow(" Type:
			 * "+(adjacentAgent.type()+1)+" Strategy: "+adjStrategy+"\n");
			 */
          ...
          /* TEMPTATION */
        } else if (agentPDAction == PD_DEFECT && adjacentAgent.agentPDAction == PD_COOPERATE) {
          energy += ComplexEnvironment.PD_PAYOFF_TEMPTATION; adjacentAgent.energy += ComplexEnvironment.PD_PAYOFF_SUCKER;
          
        /*
		 * ui.writeToTextWindow(" Agent "+getID()+"
		 * ("+position.v[0]+","+position.v[1]+") DEFECTED and gained
		 * "+ComplexEnvironment.PD_PAYOFF_TEMPTATION+" energy unit(s)\n");
		 * ui.writeToTextWindow(" Type: "+(type()+1)+" Strategy:
		 * "+Strategy+"\n"); ui.writeToTextWindow(" Agent
		 * "+(adjacentAgent.getID())+"
		 * ("+adjacentAgent.getPosition().v[0]+","+adjacentAgent.getPosition().v[1]+")
		 * COOPERATED and gained "+ComplexEnvironment.PD_PAYOFF_SUCKER+" energy
		 * unit(s)\n"); ui.writeToTextWindow(" Type:
		 * "+(adjacentAgent.type()+1)+" Strategy: "+adjStrategy+"\n");
		 */
          /* PUNISHMENT */
        } else if (agentPDAction == PD_DEFECT && adjacentAgent.agentPDAction == PD_DEFECT) {
          energy += ComplexEnvironment.PD_PAYOFF_PUNISHMENT; 
          adjacentAgent.energy += ComplexEnvironment.PD_PAYOFF_PUNISHMENT; // $$$$$$ change from "adjacentAgent.energy += 3;" (3???) to
          										 						   //       "adjacentAgent.energy += ComplexEnvironment.PD_PAYOFF_PUNISHMENT;"  Apr 22
       /*
		 * ui.writeToTextWindow(" Agent "+getID()+"
		 * ("+position.v[0]+","+position.v[1]+") DEFECTED and gained
		 * "+ComplexEnvironment.PD_PAYOFF_PUNISHMENT+" energy unit(s)\n");
		 * ui.writeToTextWindow(" Type: "+(type()+1)+" Strategy:
		 * "+Strategy+"\n"); ui.writeToTextWindow(" Agent
		 * "+(adjacentAgent.getID())+"
		 * ("+adjacentAgent.getPosition().v[0]+","+adjacentAgent.getPosition().v[1]+")
		 * DEFECTED and gained "+ComplexEnvironment.PD_PAYOFF_PUNISHMENT+"
		 * energy unit(s)\n"); ui.writeToTextWindow(" Type:
		 * "+(adjacentAgent.type()+1)+" Strategy: "+adjStrategy+"\n");
		 */
        ...
        /*
		 * boolean tempt = false; // energy of both agents is summed up for the
		 * transaction int Epool = energy + adjacentAgent.energy; // _wasteGain +=
		 * adjacentAgent.energy; if(agentPDAction == 0) cooperate = true; else
		 * cooperate = false;
		 * 
		 * if (adjacentAgent.energy > energy && food_bias){ tempt = true; if
		 * (agentPDAction == 0){ cooperate = true; float rnd =
		 * cobweb.globals.random.nextFloat(); if (rnd > 0.8) cooperate = false;
		 * }else if(agentPDAction == 1){ cooperate = false; float rnd =
		 * cobweb.globals.random.nextFloat(); if (rnd > 0.8) cooperate = true; } } //
		 * if agent is a cooperator if (cooperate ){ // both cooperate
		 * if(adjacentAgent.agentPDAction == 0){ energy = (int)(Epool/2); if
		 * ((energy - adjacentAgent.energy) > (int)(Epool/2)) { _wasteLoss -=
		 * ((energy - adjacentAgent.energy) - (int)(Epool/2));
		 * info.useOthers(agentType, ((energy - adjacentAgent.energy) -
		 * (int)(Epool/2))); } else { _wasteGain -= (int)(Epool/2) - (energy -
		 * adjacentAgent.energy) ; info.addOthers(agentType, ((energy -
		 * adjacentAgent.energy) - (int)(Epool/2)));
		 *  } }else{ energy -= (int)(energy/2); _wasteLoss -= (int)(energy/2);
		 * info.useOthers(agentType, (int)(energy/2)); //remeber the cheater
		 * photo_memory[photo_num % memory_size] = othersID; } } else
		 * if(!cooperate){ if(adjacentAgent.agentPDAction == 0){ energy +=
		 * (int)((adjacentAgent.energy)/2); _wasteGain -=
		 ...
		 * info.useOthers(agentType, (int)(energy*0.75)); photo_memory[photo_num %
		 * memory_size] = othersID; } } if(photo_num % memory_size == 0){
    ...
    if (energy < breedEnergy) { pregnant = false; breedPartner = null; } 
  ...
  public void playPD() {
    ...
    if(pdTitForTat) { // if true then agent is playing TitForTat
      ...
      agentPDStrategy = 0; // $$$$$$ added to ensure Strategy is set to 0, i.e. probability.  Apr 22
      ...
      if (rnd > coopProb) agentPDAction = 1; // agent defects depending on
											// probability
    }
  ...
  private boolean canBroadcast() {
    if (energy > broadcastEnergyMin) return true;
    return false;
  }
  ...
  private void poop(/* cobweb.Environment.Location l */) {
    ...
    for (int i = 0; i < dirList.length; i++) {
      ...
      if (foo != null &&
        ...
        ComplexEnvironment.addWaste(currTick, foo.v[0], foo.v[1], wasteInit, wasteRate);
        wasteAdded = true;
	 ...
	 * replace a food tile with a waste tile... /* This function is assumed to
	 * add a waste tile! That is, this function assumes an existence of at least
	 * one food tile that it will be able to replace with a waste tile. Nothing
	 * happens otherwise.
	 */
    if (!wasteAdded) {
      for (int i = 0; i < dirList.length; i++) {
  ...
  void broadcastFood(cobweb.Environment.Location loc) { // []SK
    ...
    new CommPacket(CommPacket.FOOD,id,message,energy,broadcastEnergyBased,broadcastFixedRange);
    // new CommPacket sent
    /*
  ...
  void broadcastCheating(cobweb.Environment.Location loc) { // []SK
    ...
    new CommPacket(CommPacket.CHEATER,id,message,energy,broadcastEnergyBased,broadcastFixedRange);
    // new CommPacket sent
    energy -= broadcastEnergyCost; // Deduct broadcasting cost from energy
  } 
...
// }
  ...
  void receiveBroadcast() {
    ...
    switch (type) {
      case CommPacket.FOOD: receiveFoodBroadcast(commPacket); break;
      case CommPacket.CHEATER: receiveCheatingBroadcast(commPacket); break;
      /**
		 * TODO Add new broadcast types
		 */
      default: System.out.println("Unrecognized broadcast type");
    }
  }
  ...
  void turnRight() {
    ...
    if (energy <= 0) die();
    if ( !pregnant && asexFlag && energy >= breedEnergy && asexualBreedChance != 0.0 && cobweb.globals.random.nextFloat() < asexualBreedChance) { 
      pregPeriod = pregnancyPeriod;
      pregnant = true;
    }
    if (pregnant) { pregPeriod --; }
  ...
  ComplexAgent (cobweb.Environment.Location pos, ComplexAgent parent, ComplexAgent parent2, int strat) {
    super(pos, new GeneticController((GeneticController)parent.controller, (GeneticController)parent2.controller, parent.mutationRate));
    InitFacing();  
    
    try {
        genetic_code 
          = new GeneticCode(GeneticCode.createGeneticCodeMeiosis(parent.getGeneticSequence(), 
                                                                 parent2.getGeneticSequence()));
        if (cobweb.globals.random.nextFloat() <= parent.mutationRate) { // parent.mutationRate?
          genetic_code = new GeneticCode(GeneticCode.mutate(genetic_code.getGeneticInfo(),
                                                            cobweb.globals.random.nextInt(GeneticCode.NUM_BITS)));
        }
      } catch (GeneticCodeException e) {
        System.out.println(e.getMessage());
     }
    copyConstants(parent);
    info = ((ComplexEnvironment)(pos.getEnvironment())).addAgentInfo( agentType, parent.info, parent2.info, strat );
    
    // Determine the agent's genetic code.
    
    ((ComplexEnvironment)pos.getEnvironment()).getColorizer().colorAgent(this);
    info.setGeneticCode(this.getGeneticSequence(), this.getGeneticCode().getGeneticColour());  // Records the agent's
														// genetic code.
    GATracker.addAgent(this); // Add this agent to the GATracker's list of agent (presumably all living agents)
  }
  ...
  ComplexAgent (cobweb.Environment.Location pos, ComplexAgent parent, int strat) {
    super(pos, new GeneticController((GeneticController)parent.controller, parent.mutationRate));
    InitFacing();    
    try {
  ...
  public void copyConstants( ComplexAgent p ) {
    setConstants(p.agentType,
                 p.agentPDAction,
                 p.memory_size,
                 p.food_bias,
                 default_energy[p.agentType],
                 default_foodEnergy[p.agentType],
                 ...
                 default_mutationRate[p.agentType],
                 p.colorful,
                 ...
                 default_sexualPregnancyPeriod[p.agentType],
                 p.agingMode,
                 default_agingLimit[p.agentType],
                 default_agingRate[p.agentType],
                 p.wasteMode,
                 p.wastePen,
                 p.wasteGain,
                 p.wasteLoss,
                 ...
                 p.wasteInit,
                 p.pdTitForTat,
                 default_pdCoopProb[p.agentType],
                 p.broadcastMode,
                 p.broadcastEnergyBased,
                 default_broadcastFixedRange[p.agentType],
                 default_broadcastEnergyMin[p.agentType],
                 default_broadcastEnergyCost[p.agentType],
                 p.plantstoeat,
                 p.agentstoeat,
                 p.agentPDStrategy, // 1 == cheater, else cooperator
                 p.agentPDAction, // The agent's action;     $$$$$ this parameter seems useless, there is another PDaction parameter above already.  Apr 18
  ...
  ComplexAgent ( int agentT,
                cobweb.Environment.Location pos,
                int action,
                int memory,
                boolean foodBstrat,
                int initE,
                ...
                boolean broadcastEnergyBased,
                int broadcastFixedRange,
                int broadcastEnergyMin,
                int broadcastEnergyCost,
                int plants2eat[],
                int agents2eat[],
                int agentPDStrategy, // Tit-for-tat or probability based
                int agentPDAction, // The agent's action; 1 = cheater, else cooperator
    ...
    super(pos, new GeneticController(memoryB,commB));
    InitFacing();
    try { 
      genetic_code = new GeneticCode(genetic_sequence); // Initialize the
														// genetic code.
    } catch (GeneticCodeException e) {
      System.out.println(e.getMessage());
    }
    setConstants(agentT,
                 ...
                 plants2eat,
                 agents2eat,
                 agentPDStrategy, // Tit-for-tat or probability based
                 agentPDAction, // The agent's action; 1 = cheater, else
								// cooperator 		$$$$$ this parameter seems useless, there is another PDaction parameter above already.  Apr 18
                 lastPDMove,// Remember the opponent's move in the last game
                 genetic_sequence); // The genetic sequence of agent.
    
// assign an ID
// id = rand.nextLong();
    int[] agent_rgb_colour_values = this.getGeneticCode().getGeneticColour();
    Color agent_colour = new Color(agent_rgb_colour_values[GeneticCode.RED_PIXEL_COLUMN],
    ...
    info = ((ComplexEnvironment)(pos.getEnvironment())).addAgentInfo( agentType, action );
    info.setGeneticCode(this.getGeneticSequence(), this.getGeneticCode().getGeneticColour()); // Records the agent's
													// genetic code.
    GATracker.addAgent(this); // Add this agent to the GATracker's list of agent (presumably all living agents)
  ...
  public void setConstants(int agentT,
                           int action,
                           int memory,
                           boolean foodBstrat,
                           int initE,
                           ...
                           int agents2eat[],
                           int agentPDStrategy, // Tit-for-tat or probability based 
												// $$$$$ a redundant parameter for now (keep it for future using), as we already have boolean "pdTitForTat".  Apr 18
                           int agentPDAction, // The agent's action; 1 =
												// cheater, else cooperator     $$$$$ this parameter seems useless, there is another PDaction parameter above already.  Apr 18
    ...
    agentType = agentT;
    this.agentPDAction = action; // $$$$$$ change from "agentPDAction == action;", for agentPDAction was duplicated to the parameter with the same name. Apr 18
    memory_size = memory;
    ...
    // $$$$$$ Modified the following block. we are not talking about RESET here.  Apr 18
    /*agentPDStrategy = 1; // Tit-for-tat or probability based
    agentPDAction = -1; // The agent's action; 1 = cheater, else cooperator      $$$$$ this parameter seems useless, there is another PDaction parameter above already.  Apr 18
    lastPDMove = -1; // Remember the opponent's move in the last game */
    this.agentPDStrategy = agentPDStrategy;  // above all, sometimes a new agent need copy PDStrategy from its parent.  See copyConstants( ComplexAgent p )
    this.lastPDMove = lastPDMove; // "KeepOldAgents" need pass this parameter. (as a reasonable side effect, the parameter of a parent would also pass to its child)  
    					  		  // See ComplexEnvironment.load(cobweb.Scheduler s, Parser p/* java.io.Reader r */) @ if (keepOldAgents[0]) {...
    ...
    Iterator<String> it = PhenotypeMaster.genetic_linkages.keySet().iterator();    
    String s = "";
    while (it.hasNext()) {
      ...
      // Coefficient = absolute value of 2*sin(x), x being attribute value in degrees
      double coefficient = 2*Math.abs(Math.sin(gene_value*Math.PI/180));
      ...
      if (o instanceof Float) {
    	...
    	/* This is really supposed to round a number and turn it into
    	 * an int. I can't seem to find a neat way to do this :/ */
        mutables.put(s, (new Long(Math.round(((Integer) o)*coefficient))).intValue());
      }  
    }
    ...
    sexualPregnancyPeriod = (Integer) mutables.get("Pregnancy Period - 2 parents");
